...
class Pipeline:
    @staticmethod
    def set_table_dtypes(df):  # dtypeを統一する
    ...
    @staticmethod
    def handle_dates(df):  # Dの特徴量をdate_decisionからの差分日数に変更する
        for col in df.columns:
            if col[-1] in ("D",):
    ...
    @staticmethod
    def filter_cols(df):  # 特徴量について、nullが95%以上のものとnuniqueが1<nunique<200に該当しないものを削除する
        for col in df.columns:
...
class Aggregator:
    @staticmethod
    def num_expr(df):  # PとAの特徴量について、最大値を取り出して特徴量に追加
    ...
    @staticmethod
    def date_expr(df):  # Dの特徴量について、最大値を取り出して特徴量に追加
    ...
    @staticmethod
    def str_expr(df):  # Mの特徴量について、最大値を取り出して特徴量に追加
    ...
    @staticmethod
    def other_expr(df):  # TとLの特徴量について、最大値を取り出して特徴量に追加
    ...
    @staticmethod
    def count_expr(df):  # num_groupについて、最大値を取り出して特徴量に追加
    ...
    @staticmethod
    def get_exprs(df):  # 上記の関数を実行して、結果を返す
...
def read_file(path, depth=None):  # dtypeの統一、case_idでグループ化した上での最大特徴量の追加
...
def read_files(regex_path, depth=None):  # dtypeの統一、複数dfの縦方向での結合、case_idでグループ化した上での最大特徴量の追加
    chunks = []
